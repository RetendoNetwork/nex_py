from errors import error_codes, error_names
import random
import datetime, time

import streams
from prudp import PRUDPPacketFlags, PRUDPPacketTypes, PRUDPTransportTypes
...
class Structure:
	...
	def decode(self, stream):
		...
		for cls in hierarchy:
			if 0:
				max_version = cls.max_version(self)
...
class SequenceMgr:
	...
	def assign(self, packet):
		...
		if packet.type == PRUDPPacketTypes.PING_PACKET:
...
class SlidingWindow:
	...
	def update(self, packet):
		...
		if packet.packet_id < self.next or packet.packet_id in self.packets:
			logger.info("Received duplicate packet: %s", packet)
		else:
