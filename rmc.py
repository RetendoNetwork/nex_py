...
class RMC:
    ...
    def decode_verbose(self, data: bytes) -> Optional[Exception]:
        ...
        try:
            ...
            self.protocol_name = stream.read_string()
            self.is_request = stream.read_bool()
            if self.is_request:
                self.call_id = stream.read_uint32_le()
                self.method_name = stream.read_string()
                self.version_container = stream.read_class_version_container()
                self.parameters = stream.read_remaining()
            else:
                self.is_success = stream.read_bool()
                if self.is_success:
    ...
    def encode_packed(self) -> bytes:
        ...
        if self.is_request:
            ...
            if self.parameters:
                stream.write_bytes(self.parameters)
        else:
        ...
        message.write_bytes(serialized)
    ...
    def encode_verbose(self) -> bytes:
        ...
        stream.write_string(self.protocol_name)
        stream.write_bool(self.is_request)
        if self.is_request:
            stream.write_uint32_le(self.call_id)
            stream.write_string(self.method_name)
            if self.version_container:
                stream.write_class_version_container(self.version_container)
            else:
                stream.write_uint32_le(0)  # Fail safe
            if self.parameters:
                stream.write_bytes(self.parameters)
        else:
            stream.write_bool(self.is_success)
